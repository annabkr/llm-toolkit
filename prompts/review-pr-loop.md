# review-pr loop orchestration

Use this prompt to run escalating review agents against a PR, automatically fixing issues and progressively widening the review lens until the change converges.
...
|-------|------|-------------|------|
| 1 | `code` | `~/.claude/skills/review-pr/passes/01-code.md` | In-the-small: line-level bugs and quality |
| 2 | `gaps` | `~/.claude/skills/review-pr/passes/02-gaps.md` | In-the-small: what the first pass missed |
| 3 | `design` | `~/.claude/skills/review-pr/passes/03-design.md` | In-the-large: does this design belong here? |
| 4 | `architecture` | `~/.claude/skills/review-pr/passes/04-architecture.md` | In-the-large: is this the right thing to build? |
| 5 | `convergence` | `~/.claude/skills/review-pr/passes/05-convergence.md` | Full picture: is this as good as we can make it? |
...
1. **Read** the agent prompt file for the current pass (starting with `01-code.md`).
